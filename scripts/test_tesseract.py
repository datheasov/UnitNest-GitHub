...
try:
    cnn_model = tf.keras.models.load_model("models/meter_classifier3")
...
def run_tesseract_ocr(image, config=""):
    """
    Run Tesseract OCR with given config. Returns a list of (text, confidence).
    Confidence is in the range [0..100], or -1 for invalid.
    """
    # Convert to data for bounding box + confidence info
    data = pytesseract.image_to_data(image, config=config, output_type=Output.DICT)
    extracted_texts = []
    for i in range(len(data["text"])):
        txt = data["text"][i].strip()
        conf = data["conf"][i]
        # Filter out empty text or invalid conf
        if txt and conf != '-1':
            extracted_texts.append((txt, float(conf)))
...
def extract_4to5_digits(tess_data, min_confidence=80.0):
    """
    Given a list of (text, confidence) from Tesseract,
    return the best 4- or 5-digit candidate with confidence >= min_confidence.
    """
    candidates = []
    for txt, conf in tess_data:
    ...
    if candidates:
        ...
        return postprocess_ocr_result(best[0]), best[1]
    return "N/A", 0.0
    ...
    try:
        ...
        for result in detected_boxes:
            ...
            for variant_name, preprocessed in variants.items():
                # Ensure the image is 3-channel if needed
                ...
                if SAVE_IMAGES:
                    variant_crop_path = os.path.join(
                        OUTPUT_DIR, f"{base_filename}_box{box_idx}_{variant_name}_gray.png"
                    )
                    # If it's grayscale, just save as is
                    if len(preprocessed.shape) == 2:
                        cv2.imwrite(variant_crop_path, preprocessed)
                    else:
                        gray_for_save = cv2.cvtColor(preprocessed_bgr, cv2.COLOR_BGR2GRAY)
                        cv2.imwrite(variant_crop_path, gray_for_save)

                # =================== Tesseract: Three Calls (Mimicking Specialized, Default, Fine-tuned) ===================
                # 1) "Special" Tesseract config
                tess_data_special = run_tesseract_ocr(preprocessed_bgr, config="--psm 7")
                text_special, conf_special = extract_4to5_digits(tess_data_special)

                # 2) "Default" Tesseract config
                tess_data_default = run_tesseract_ocr(preprocessed_bgr, config="--psm 6")
                text_default, conf_default = extract_4to5_digits(tess_data_default)

                # 3) "Fine-tuned" Tesseract config
                tess_data_finetuned = run_tesseract_ocr(preprocessed_bgr, config="--psm 8")
                text_finetuned, conf_finetuned = extract_4to5_digits(tess_data_finetuned)
...
if __name__ == "__main__":
    ...
    with open("v5810_10075100GF_tesseract2.json", "w") as f:
